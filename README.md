...
首次启动只需隐藏输入一个从 QVeris 控制台创建的上游 API key。工具会另外按 `sk-` 官方样式安全随机生成一个主代理 API Key、启动轻量容器，并打开已连接的管理页。即使用户第一次直接打开裸管理地址，快速安装模式也会让第一个浏览器自动认领并连接，无需填写代理 API Key。接入 Vibe 等应用时使用代理 API Key，不再填写上游 QVeris API key。两种 Key、账号配置和 SQLite 状态都放在 Docker named volumes 中，不会写入 Git 仓库，也不需要手动创建宿主机 secret 目录。
...
管理页默认是 http://127.0.0.1:18081/admin/。快速安装模式下，该安装的第一个浏览器直接打开管理地址即可自动认领；认领成功后使用 HttpOnly、SameSite=Strict 的签名 Cookie 自动恢复，同一浏览器关闭后重新打开仍会连接。签名 Cookie 滑动有效 180 天，代理 API Key 不写入 localStorage，点击“断开连接”会清除浏览器会话，首次认领也不会再次开放。其他浏览器可重新运行启动脚本，使用 60 秒有效且只能使用一次的连接票据，或展开“手动连接”。“运行状态”首屏的“接入应用”区域会同时显示 API Base URL 和主代理 API Key，可显示、隐藏、分别复制或一键复制全部接入配置。

在管理页的“账号配置”中先点击“测试”验证首个账号，再点击“添加账号”，逐个录入其他 QVeris API key 并保存。登录 QVeris 网站本身不会自动把账号加入代理池；只有在这里添加并保存的账号才参与路由。所有代理 API Key 都连接同一个多账号池；搜索、工具发现和执行请求会在可用账号间按权重轮询，同一 session/search/execution 会保持账号亲和。各账号额度仍分别记录和显示，不会合并成一个总余额。

“代理 Key”页签可按用户或应用创建任意数量的 `sk-` Key。每个 Key 可独立设置启停、累计请求上限、每分钟请求上限、最大并发和到期时间，并可随时编辑、重置用量或删除。新 Key 的完整明文只在创建成功时显示一次；SQLite 只保存 SHA-256 哈希和脱敏前后缀，因此创建后应立即复制到目标应用。主 Key 为兼容现有接入保留，不能删除；管理认证只接受主 Key，普通托管 Key 只能调用代理 API，不能修改自己的限制。
...
- 主代理 API Key 同时用于管理页和 API 调用，请像密码一样保存，不要发到群聊、截图或公开仓库。额外创建的托管 Key 只用于 API 调用，仍应按凭据妥善保存。
...
本地管理路由包括 /admin/v1/accounts、/admin/v1/config、/admin/v1/config/validate、/admin/v1/accounts/{id}/test、DELETE /admin/v1/accounts/{id}、/admin/v1/refresh-credits、/admin/v1/reload-accounts，以及 /admin/v1/proxy-keys 的创建、查询、编辑、删除和用量重置接口。管理接口返回脱敏状态；托管 Key 的明文只在创建响应中出现一次。
