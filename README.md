...
在管理页的“账号配置”中先点击“测试”验证首个账号，再点击“添加账号”，逐个录入其他 QVeris API key 并保存。每个账号都有可随时修改的显示名称；内部 ID 自动生成并保持稳定，避免改名影响路由和历史状态。登录 QVeris 网站本身不会自动把账号加入代理池；只有在这里添加并保存的账号才参与路由。所有代理 API Key 都连接同一个多账号池；搜索、工具发现和执行请求会在可用账号间按权重轮询，同一 session/search/execution 会保持账号亲和。各账号额度仍分别记录和显示，不会合并成一个总余额。
...
- 默认代理 API Key 同时用于管理页和 API 调用，请像密码一样保存，不要发到群聊、截图或公开仓库。额外创建的托管 Key 只用于 API 调用，仍应按凭据妥善保存。
